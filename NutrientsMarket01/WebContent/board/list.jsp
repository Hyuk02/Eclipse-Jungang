...
function checkForm(){
	...
	//writeForm으로 이동
	location.href="./BoardWriteFormAction.bo?id=<%=sessionId%>";
}
</script>    
...
<div class="jumbotron">
  <div class="container">
  	<h1 class="display-3">후기게시판</h1>
  </div>
</div>
<div class="container">
	<form action="<c:url value="./BoardListAction.bo"/>" method="post">
  <div>
  ...
  <tr>
   <th>번호</th><th></th><th>제목</th><th>제품</th><th>작성일</th><th>조회</th><th>글쓴이</th>
  </tr>                                         
  <%
  	  ...
  	  <tr>	   	  	
  		<td><%=notice.getNum() %></td>
  		<td><img src="./resources/images/<%=notice.getFileName()%>" style="height:40px"/></td>
  		<td><a href="./BoardViewAction.bo?num=<%=notice.getNum()%>&pageNum=<%=pageNum%>"><b><%=notice.getSubject() %></b></a></td>
  		<td><%=notice.getProduct() %></td>
  		<td><%=notice.getWrite_day() %></td>
  		<td><%=notice.getHit() %></td>  
  		<td><%=notice.getId() %></td>
  	   </tr>	  
  	 <% }%>
  	...
  	<ul class="pagination">
  		...
  		<c:if test="${currentBlock>1}">
  			<li class="page-item"><a class="page-link" href="<c:url value="./BoardListAction.bo?pageNum=${startPage-1}&items=${items}&text=${text}"/>">Previous</a></li>
  		</c:if>
  		<!-- 현재 세그먼트 블록내에서의 페이지 리스트 -->
  	    <c:forEach var="i" begin="${startPage}" end="${endPage}">
  	       <li <c:if test="${pageNum==i}">class="page-item active"</c:if>><a class="page-link" href="<c:url value="./BoardListAction.bo?pageNum=${i}&items=${items}&text=${text}"/>">
  	         <c:choose>
  	    ...
  	    <c:if test="${currentBlock<total_segment}">
  			<li class="page-item">
  			<a class="page-link" href="<c:url value="./BoardListAction.bo?pageNum=${endPage+1}&items=${items}&text=${text}"/>">Next</a></li>
  		</c:if>
  		<c:if test="${currentBlock>=total_segment}">
