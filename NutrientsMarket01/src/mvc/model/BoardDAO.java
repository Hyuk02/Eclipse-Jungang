...
public class BoardDAO {
	  ...
	  try {
		  //db연결맺기
		   ...
		   while(rs.absolute(index)) {//한행씩 전진
		      ...
		      board.setWrite_day(rs.getString("write_day"));
	...
	public void insertBoard(BoardDTO board) {
		...
		try {
			  con=DBConnection.getInstance().getConnection();
	String sql="insert into board(id,subject,product,content,write_day,hit,fileName) values(?,?,?,?,?,?,?)";
	          //쿼리객체 새성
			  pstmt=con.prepareStatement(sql);		  
	          //파라미터 설정
	          pstmt.setString(1, board.getId());
			  pstmt.setString(2, board.getSubject());
			  pstmt.setString(3, board.getProduct());
			  pstmt.setString(4, board.getContent());
			  pstmt.setString(5, board.getWrite_day());
	...
	public void updateBoard(BoardDTO board) {
		...
		try {
			  con=DBConnection.getInstance().getConnection();
	String sql="update board set subject=?,content=?,regist_day=?,fileName=? where num=?";
	          //쿼리객체 새성
			  pstmt=con.prepareStatement(sql);		  
	          //파라미터 설정
			  pstmt.setString(1, board.getSubject());
			  pstmt.setString(2, board.getContent());
			  pstmt.setString(3, board.getWrite_day());
